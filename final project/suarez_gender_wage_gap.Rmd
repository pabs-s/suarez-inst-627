...
#don't run this codeblock until you've called merged data from the next section
merged_data |>
...
In this section of the project I merged some of the dataframes I created earlier in the project to be able to conduct regression analyses of the different variables isolated in the questions above. Given that the year column in each dataframe starts and ends with different years, I used the reduce and intersect functions to merge them based on the same starting and ending year, which is from 1982 to 2020. One shortcoming to this approach is that I miss out on a few years post-Covid, but this was the most practical method to joining the various dataframes.
...
# Subset each dataset to include only the common years
...
I took each column from my merged dataset representing hourly median wages and created a for loop to convert them to a weekly median salary (assuming a 40-hour workweek).
...
# Create new columns with standardized values in merged_data
...
subset_data_2020 <- merged_data[merged_data$year == 2020, ]
...
Other factors that can be taken into consideration should I continue working on this project are the number of years of relevant work experience, geographic location and cost of living, and marital status.
